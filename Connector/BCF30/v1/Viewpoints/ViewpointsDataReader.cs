...
public class ViewpointsDataReader : TypedAsyncDataReaderBase<ViewpointsDataObject>
{
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        IEnumerable<ViewpointsDataObject>? viewpoints = null;

        try
        {
            var response = await _apiClient.GetBcf30Viewpoints(
                projectId,
                topicId,
            ...
            {
                throw new Exception($"Failed to retrieve BCF 3.0 viewpoints. API StatusCode: {response.StatusCode}");
            }

            viewpoints = response.Data;
        }
        catch (HttpRequestException exception)
        {
            _logger.LogError(exception, 
                "Exception while retrieving BCF 3.0 viewpoints for project {ProjectId} and topic {TopicId}", 
                projectId, topicId);
            throw;
        }
