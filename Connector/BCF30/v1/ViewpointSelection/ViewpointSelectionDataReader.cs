...
public class ViewpointSelectionDataReader : TypedAsyncDataReaderBase<ViewpointSelectionDataObject>
{
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        ViewpointSelectionDataObject? selection = null;

        try
        {
            var response = await _apiClient.GetBcf30ViewpointSelection(
                projectId,
                topicId,
                viewpointId,
            ...
            {
                throw new Exception($"Failed to retrieve BCF 3.0 viewpoint selection. API StatusCode: {response.StatusCode}");
            }
        ...
        catch (HttpRequestException exception)
        {
            _logger.LogError(exception, 
                "Exception while retrieving BCF 3.0 viewpoint selection for project {ProjectId}, topic {TopicId}, and viewpoint {ViewpointId}", 
